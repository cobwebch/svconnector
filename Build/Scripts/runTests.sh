...
case ${TEST_SUITE} in
    ...
    functional)
        CONTAINER_PARAMS=""
        COMMAND=(.Build/bin/phpunit -c .Build/vendor/typo3/testing-framework/Resources/Core/Build/FunctionalTests.xml Tests/Functional/ --exclude-group not-${DBMS} --display-skipped ${EXTRA_TEST_OPTIONS} "$@")
        case ${DBMS} in
            mariadb)
